@echo off
set "GITHUB_PROXY=https://gh-proxy.com/"
set "CUSTOM_BAMBU_API_URL=https://api.github.com/repos/greghesp/ha-bambulab/releases/latest"
set "CUSTOM_XIAOMI_API_URL=https://api.github.com/repos/XiaoMi/ha_xiaomi_home/releases/latest"
set "CUSTOM_BAMBU_URL=https://github.com/greghesp/ha-bambulab/releases/latest/download/bambu_lab.zip"
set "CUSTOM_XIAOMI_URL=https://github.com/XiaoMi/ha_xiaomi_home/releases/latest/download/xiaomi_home.zip"
    call :log "δҵԶѹ"
    call :log "ǰ..."

    set "CUSTOM_XIAOMI_URL="
    for /f "delims=" %%i in ('powershell -Command "$ErrorActionPreference = 'SilentlyContinue'; try { ((Invoke-RestMethod -Uri '!CUSTOM_XIAOMI_API_URL!').assets | Where-Object { $_.name -eq '!CUSTOM_XIAOMI_NAME!' } | Select-Object -First 1 -ExpandProperty browser_download_url) } catch { '' }"') do set "CUSTOM_XIAOMI_URL=%%i"
    if "!CUSTOM_XIAOMI_URL!"=="" set "CUSTOM_XIAOMI_URL=https://github.com/XiaoMi/ha_xiaomi_home/releases/latest/download/!CUSTOM_XIAOMI_NAME!"

    set "CUSTOM_BAMBU_URL="
    for /f "delims=" %%i in ('powershell -Command "$ErrorActionPreference = 'SilentlyContinue'; try { ((Invoke-RestMethod -Uri '!CUSTOM_BAMBU_API_URL!').assets | Where-Object { $_.name -eq '!CUSTOM_BAMBU_NAME!' } | Select-Object -First 1 -ExpandProperty browser_download_url) } catch { '' }"') do set "CUSTOM_BAMBU_URL=%%i"
    if "!CUSTOM_BAMBU_URL!"=="" set "CUSTOM_BAMBU_URL=https://github.com/greghesp/ha-bambulab/releases/latest/download/!CUSTOM_BAMBU_NAME!"

    powershell -Command "try { (New-Object System.Net.WebClient).DownloadFile('!GITHUB_PROXY!!CUSTOM_XIAOMI_URL!', '!BIN_DIR!\!CUSTOM_XIAOMI_NAME!') } catch { Write-Host 'DOWNLOAD_FAILED'; exit 1 }" >> "!LOG_FILE!" 2>&1
    if errorlevel 1 (
        echo.
        call :log "[!] Xiaomi custom package download failed."
        call :log "Xiaomi package URL: !CUSTOM_XIAOMI_URL!"
        pause
        goto main
    )
    powershell -Command "try { (New-Object System.Net.WebClient).DownloadFile('!GITHUB_PROXY!!CUSTOM_BAMBU_URL!', '!BIN_DIR!\!CUSTOM_BAMBU_NAME!') } catch { Write-Host 'DOWNLOAD_FAILED'; exit 1 }" >> "!LOG_FILE!" 2>&1
        call :log "[!] ֲļʧܡֶء"
        call :log "Bambu package URL: !CUSTOM_BAMBU_URL!"
    call :log "Զأ3..."
:: ########## 核心：获取脚本所在目录的绝对路径 ##########
set "SCRIPT_DIR=%~dp0"
...
if not defined board (
    call :log "[!] Invalid selection, returning to main menu."
    timeout /t 2 /nobreak >nul
    goto main
)
